...
from ._palettes import palette_28, palette_56
_VarNames = Union[str, Sequence[str]]


def _marsilea_figure(m: ma.ClusterBoard):
    """Render a marsilea plot and return its matplotlib Figure."""
    m.render()
    fig = getattr(m, "figure", None)
    if fig is not None:
        return fig
    hax = m.get_main_ax()
    if not hasattr(hax, "get_figure"):
        hax = np.array(hax).flatten()[0]
    return hax.get_figure()
...
def dotplot(
    ...
    *,
    ...
    gene_group_colors: Optional[Dict[str, str]] = None,
    **kwds,
    ...
    if 'gene_groups' in locals():
        ...
        if gene_group_colors is not None:
            if color_dict is None:
                color_dict = {}
            color_dict.update(gene_group_colors)
...
def rank_genes_groups_df(
    ...
    log2fc_min: Union[float, None] = None,
) -> pd.DataFrame:
